...
- Python 3.9+
- 4 GB RAM (CPU) or 4 GB RAM + GPU
- Optional: Docker, NVIDIA GPU
...
### Docker (GPU — 3–5× faster)

```bash
# Requires NVIDIA GPU + nvidia-docker
docker-compose -f docker-compose-gpu.yml up -d
docker-compose -f docker-compose-gpu.yml logs -f   # should show "Using device: cuda"
```
...
python app.py                      # dev server (cross-platform)
# OR, on Linux for production:
gunicorn -c gunicorn.conf.py app:app
```

Visit http://localhost:5001

On Windows, `gunicorn` won't work (it depends on `fcntl`). Either run `python app.py` directly, or use a Windows-compatible WSGI server such as `waitress`:
...
| Direction | Model |
|---|---|
| `latin2thaana` | [`str33t/dhivehi-byt5-latin2thaana-keymap-v1`](https://huggingface.co/str33t/dhivehi-byt5-latin2thaana-keymap-v1) |
| `thaana2latin` | [`Neobe/dhivehi-byt5-thaana2latin-v1`](https://huggingface.co/Neobe/dhivehi-byt5-thaana2latin-v1) |

The `latin2thaana` model warm-starts from [`Neobe/dhivehi-byt5-latin2thaana-v1`](https://huggingface.co/Neobe/dhivehi-byt5-latin2thaana-v1); only the decoder was retargeted to emit ASCII keymap output. `keymap.py:keymap_to_thaana()` is the post-processor — pure Python, deterministic, ~10 lines of logic. See [How the model was built](#how-the-model-was-built) for the full writeup.
...
```json
{
  ...
  "direction": "latin2thaana"
}
```

`direction` is `latin2thaana` (default) or `thaana2latin`.

**Response** — Server-Sent Events
```
data: {"status": "0%", "progress": 0}
data: {"status": "33%", "thaana": "ސަލާމް", "partial": true, "progress": 33}
data: {"status": "100%", "thaana": "ސަލާމް ދިވެހި ރާއްޖެ", "partial": false, "progress": 100}
```

### GET `/ready`

Returns `200 {"status": "ready"}` once both models are loaded; `503 {"status": "loading"}` during warmup. Use as a healthcheck or load-balancer readiness probe.

## Configuration

### Linux production server (`gunicorn.conf.py`)
```python
workers = 4      # ~2 GB RAM per worker
threads = 2
timeout = 180
```
...
Edit `dhivehi-transliteration.service` with your paths, then:
```bash
sudo cp dhivehi-transliteration.service /etc/systemd/system/
sudo systemctl daemon-reload
sudo systemctl enable --now dhivehi-transliteration
```
...
Flask 3.1.2 · PyTorch 2.8.0 · Transformers 4.57.6 · Gunicorn 21.2.0 (Linux) · ByT5-small (299.6M params)

## License

MIT — see [LICENSE](LICENSE). The HuggingFace model card lists Apache 2.0 (inherited from the ByT5 base); this repo's code is MIT.
...
We retarget the decoder to emit ASCII-encoded Thaana via a deterministic, lossless 1-to-1 mapping based on the Standard Phonetic ("Segha") Dhivehi keyboard layout [3]. A pure-function post-processor reconstructs Thaana from the model's ASCII output. End-to-end user experience is unchanged, but each output character costs one decoder step instead of three.
...
The retrained model delivers a **3.02× decoder speedup** on GPU (2.07× on CPU), passes 3 of 4 documented short-input failure cases exactly, and matches the baseline on realistic sentences.
...
The technique combines three components: a keymap that gives a 1-byte ASCII encoding of every Thaana codepoint, a deterministic converter for training-target generation and inference post-processing, and a length-diverse fine-tuning corpus that addresses the short-input failure mode. The base model is `Neobe/dhivehi-byt5-latin2thaana-v1` [4] (itself fine-tuned from `google/byt5-small` on `alakxender/dhivehi-transliteration-pairs` [2]); the keymap reference is `jawish/jtk` [3].

### 2.1 Keymap choice: Segha phonetic layout
...
After applying the same `,/;/?` → Arabic punctuation normalization that the existing service already does at output time: **0 / 187,908 failures**. The converter is sound and the spec is complete.

This normalization runs upstream in the training-data prep, which is consistent with the existing service behaviour and not a behavior change.
...
### 3.2 Short-input regression cases (production decoder params)

| Input | Baseline (shipped Neobe) | New model | Expected | New result |
|---|---|---|---|---|
| `bohkuraa` | `ބޮއްކުރާ ބޮއްކުރާ ބޮއްކުރާ ބޮއްކުރާ` (×4 repeat) | `ބޮށްކުރާ` | `ބޮއްކުރާ` | 1-char error (shaviyani vs alifu) |
...
### 3.3 Realistic sentences (production decoder params)

| Input | New model output (post keymap-decode) | Match baseline? |
|---|---|---|
| `Aharennakee Dhivehi bahun vaahaka dhakkaa meeheh` | `އަހަރެންނަކީ ދިވެހި ބަހުން ވާހަކަ ދައްކާ މީހެއް` | ✓ identical |
...
**CPU vs GPU latency.** Same model, same inputs, same decoder params — only the hardware differs. CPU runs in `venv-cpu` (PyTorch CPU wheel), GPU runs in `venv` (CUDA 12.8 wheel) on the RTX 5070 Ti.
...
**End-to-end streaming latency (Flask pipeline, GPU).** A 231-word paragraph through the production pipeline (chunking → batched inference → `keymap_to_thaana` → RTL punctuation) completes in **4.87 s** total over 11 SSE events, no silent gap longer than 2.08 s.

## 4. Discussion

### 4.1 The bet paid off — both ways
...
### 4.2 Warm-starting was load-bearing
...
### 4.3 The single failure case

`bohkuraa → ބޮށްކުރާ` instead of the canonical `ބޮއްކުރާ` is a single-character substitution: `ށ` (shaviyani) vs `އ` (alifu) in position 2. Probable causes:
...
### 4.5 Generalization beyond Dhivehi
...
The model has been wired into the production Flask service and published to Hugging Face under [`str33t/dhivehi-byt5-latin2thaana-keymap-v1`](https://huggingface.co/str33t/dhivehi-byt5-latin2thaana-keymap-v1). End-to-end testing on the live service confirms identical outputs to the standalone regression eval.
...
The deferred items are real options, not loose ends: the long-concat data is on disk, and the spec for single-word augmentation is documented in §2.4. This experiment was self-contained and ships as-is.
